...
if getpass.getuser() == "root":
	...
	if os.path.exists(dnsmasq_config_dir) == False:
	...
	if len(new_block_List) > 0:
		...
		with open(dnsmasq_config_dir + config_file,"r") as handle:
			current_List = [x.strip().replace('address=/','').replace("/" + point_To,'') for x in handle.readlines()]
		...
		#write the final list to 
			with open(dnsmasq_config_dir + config_file,"w") as handle:
				...
				print str((len(final_List) - len(current_List))) + " original entries have been added"
				print str(len(final_List)) + " now in blocklist"
			#reload dnsmasq so it uses the updated list
			print "reloading dnsmasq..."
			process = subprocess.Popen("killall -s SIGHUP dnsmasq", shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
